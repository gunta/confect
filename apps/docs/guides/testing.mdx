...
`Effect.result(eff)` materialises the error channel as `Result.Result<A, E>`. Use `assertSuccess` / `assertFailure` from `@effect/vitest/utils` to assert on the result.

## Running setup code

Use `run` to execute arbitrary code with mutation-context services (like `DatabaseWriter`) for test setup. When the handler returns a value, pass a `Schema.Codec<A, B, never, never>` as the second argument for decoding.
