...
<Note>
  This page targets the `@confect/test@next` prerelease, which depends on `effect@4.0.0-beta.67`, `@effect/vitest@4.0.0-beta.67`, `convex@^1.39.0`, and `convex-test@^0.0.53`. For Effect 3 docs, see the stable `@confect/test` (latest) release.
</Note>
...
<Tip>
  `convex-test@^0.0.53` builds on the parallel-test isolation upgrade introduced in `0.0.45`: each `convexTest()` instance scopes its in-memory state via `AsyncLocalStorage`, which removes flakiness when multiple test files (or `describe` blocks) run concurrently.
</Tip>
...
When a ref's spec declares an `error` schema, the decoded error is added to the error channel of `query`, `mutation`, and `action` alongside `Schema.SchemaError`. See [Error Handling](/server/error-handling) for how to declare error schemas.
...
In Effect 4, `Effect.either` has been replaced with `Effect.result`, which materializes the error channel as a `Result.Result<A, E>` instead of `Either<E, A>`. The matching `@effect/vitest/utils` helpers are `assertSuccess` and `assertFailure` (replacing v3's `assertRight` / `assertLeft`).

## Running setup code

Use `run` to execute arbitrary code with mutation-context services (like `DatabaseWriter`) for test setup. When the handler returns a value, pass a `Schema.Codec<A, B, never, never>` as the second argument for decoding (Effect 4 unifies `Schema.Schema<A, I>` into `Schema.Codec<A, I, never, never>`).
