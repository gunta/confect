...
<Note>
  ...
  handlers do **not** currently receive a `NodeContext` in the Effect
  requirements set. The previous `FileSystem`/`Path`/`CommandExecutor`
  shim has not yet been rewired against `effect/unstable/process` and
  friends. For child-process work in the interim, reach for the new
  `effect/unstable/process` module directly inside your handler, or use
  the underlying Convex action context. `@confect/cli` ships its own
  Node shim (`packages/cli/src/internal/{fs,path,command}.ts`) for build-time
  tooling, but those are not exposed to user code.
</Note>
