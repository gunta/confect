...
Convex 1.31+ also supports the explicit-table-id form (`db.get("table", id)`) at the base API layer. When wiring plain Convex functions alongside Confect, prefer that form for tables not already exposed through a Confect `Table`.
...
<Note>
  Convex 1.32+ adds `maximumRowsRead` and `maximumBytesRead` options to
  `PaginationOptions` for bounding a single page's work. These pass
  through `paginate` unchanged.
</Note>
