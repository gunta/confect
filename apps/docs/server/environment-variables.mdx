...
<Note>
  ...
  construction time and forwards it to `ConfigProvider.fromEnv({ env })`.
  Effect 4's default `ConfigProvider.fromEnv` would read `import.meta.env`,
  which the Convex bundler cannot statically analyze; passing the env
  explicitly side-steps that bundler limitation.
</Note>
