...
<Note>
  This page targets the `@confect/js@next` prerelease, which depends on `effect@4.0.0-beta.67` and `convex@^1.39.0`. For Effect 3 docs, see the stable `@confect/js` (latest) release.
</Note>
...
The WebSocket connection is still managed as a scoped resource: it is opened when the layer is provided and closed automatically when the scope ends. In Effect 4 the layer is constructed with `Layer.effect` rather than `Layer.scoped`—the type system tracks the underlying `Scope` requirement automatically.
...
Each method returns an `Effect` that can fail with `WebSocketClientError` (defined internally as a `Schema.TaggedErrorClass` wrapping transport-level errors) or `Schema.SchemaError` (if schema encoding or decoding fails).

## Reactive queries

`reactiveQuery` subscribes to a query over the WebSocket connection and returns a `Stream` that emits a new value whenever the query result changes on the server. Internally the stream is built with Effect 4's `Stream.callback` (replacing v3's `Stream.asyncScoped`); the user-facing API is unchanged.
...
When a ref's spec declares an `error` schema, the decoded error is added to the error channel of `query`, `mutation`, `action`, and `reactiveQuery` alongside `WebSocketClientError` and `Schema.SchemaError`. See [Error Handling](/server/error-handling) for how to declare error schemas.
...
// Effect.Effect<Note, NoteNotFound | WebSocketClientError | Schema.SchemaError, WebSocketClient.WebSocketClient>
```
