...
<Note>
  This page targets the `@confect/react@next` prerelease, which depends on `effect@4.0.0-beta.67` and `convex@^1.39.0`. For Effect 3 docs, see the stable `@confect/react` (latest) release.
</Note>
...
You can also narrow with `Result.isSuccess` / `Result.isFailure` instead of pattern matching:
