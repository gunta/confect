 ...
 *
 * Effect 4 port (no more singleton Service wrapper around a Ref — the value
 * is computed once at CLI start by the caller).
 */
...
export const findConfectDirectory = Effect.gen(function* () {
  ...
  const confectDirectory = Path.join(
    Path.dirname(convexDirectory),
    "confect",
  );
